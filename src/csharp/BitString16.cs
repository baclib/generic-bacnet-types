...
public readonly record struct BitString16 : IBitString
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="BitString16"/> struct.
    /// </summary>
    /// <param name="flags">The 16-bit unsigned integer containing the bits in system-native format (LSB-first).</param>
    /// <param name="count">The number of bits to use (0-16). Bits beyond this count will be masked to zero.</param>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="count"/> is greater than 16.</exception>
    public BitString16(ushort flags, byte count)
    {
        ...
        ushort mask = count == 16 ? (ushort)0xFFFF : (ushort)((1 << count) - 1);
